(function(){
  ...
  if(!embeddedDemo){
    ...
    if(nav){
      ...
      drawer.innerHTML=navItems.map(([label,href])=>`<a href="${href}" class="${current===href?'active':''}">${label}</a>`).join('')+`<a class="demo" href="demo.html">Competition Demo · 90 sec</a>`;
    ...
    status=document.createElement('div');status.className='platform-statusbar';status.innerHTML=`<div class="platform-status-inner"><div class="platform-status-items"><span><i class="platform-dot"></i><b id="platform-market">Market View —</b></span><span id="platform-readiness">Decision Readiness —</span><span id="platform-risk">Decision Risk —</span><span id="platform-gate">Decision Gate —</span><span id="platform-oos">Prospective Validation —</span></div><a class="platform-demo-link" href="demo.html">Launch Competition Demo</a></div>`;
  ...
  Promise.allSettled([
    ...
    if(!embeddedDemo){
      ...
      if(currency){
        ...
        if(market){market.textContent='Market View '+(finalR.pair||pa.pair||'—')+' '+(finalR.canonical_bias||pa.bias||'—');market.title='ECB session '+(currency.session_date||'—')}
  ...
  if(current==='index.html'&&!embeddedDemo){
    ...
    if(heroMeta&&!document.querySelector('.platform-home-cta'))heroMeta.insertAdjacentHTML('afterend',`<div class="platform-home-cta"><a class="primary" href="demo.html">Launch 90-Second Competition Demo</a><a class="secondary" href="intelligence.html">Open FX Intelligence</a></div><div class="platform-pitch-thesis"><b>The signal is not the product.</b> The product is the decision discipline around the signal: explain it, challenge it, constrain it, trace it, and validate it prospectively.</div>`);
    const footer=document.querySelector('footer');
    if(footer&&!document.querySelector('.platform-flow')){
      const flow=document.createElement('section');flow.className='platform-flow';flow.innerHTML=`<div class="platform-flow-wrap"><div class="platform-flow-head"><div><h2>One signal. Six layers of explainable decision intelligence.</h2><p>A competition-ready product flow from market movement to prospective validation.</p></div><a class="platform-demo-link" href="demo.html">Run guided demo</a></div><div class="platform-flow-grid">
      <div class="platform-flow-card"><div class="n">01 · DETECT</div><h3>Currency Strength</h3><p>Identify relative currency pressure and form the transparent canonical ECB pair bias.</p><a href="strength.html">Open Strength →</a></div>
      <div class="platform-flow-card"><div class="n">02 · EXPLAIN</div><h3>FX Intelligence</h3><p>Test the signal against macro, yield, cross-market, news, risk, and an adversarial counter-thesis.</p><a href="intelligence.html">Open Intelligence →</a></div>
      <div class="platform-flow-card"><div class="n">03 · WATCH</div><h3>Decision Watch</h3><p>Escalate material changes while filtering noisy alert oscillation and retaining the audit trail.</p><a href="decision-watch.html">Open Watch →</a></div>
      <div class="platform-flow-card"><div class="n">04 · TRACE</div><h3>Timeline</h3><p>See exactly what changed across sessions and why the AI decision changed with it.</p><a href="timeline.html">Open Timeline →</a></div>
      <div class="platform-flow-card"><div class="n">05 · VALIDATE</div><h3>Prospective Validation</h3><p>Track immutable Fresh OOS predictions and suppress premature performance claims.</p><a href="validation.html">Open Validation →</a></div>
      <div class="platform-flow-card"><div class="n">06 · GOVERN</div><h3>Model Integrity</h3><p>Keep thresholds and weights frozen, execution off, and evidence separated from profit claims.</p><a href="validation.html">Review Integrity →</a></div>
