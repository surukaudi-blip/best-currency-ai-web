(function(){
  ...
  section.innerHTML=`<div class="wrap"><div class="stock10c-head"><div><h2>Stocks Decision Intelligence · Stage 10C/10D</h2><p>SEC official evidence + daily market context combined through transparent reasoning, then monitored under a frozen prospective model.</p></div><span class="stock10c-badge" id="stock10cBadge">LOADING MODEL STATUS</span></div><div class="stock10d-links"><a href="stocks-watch.html">Decision Watch →</a><a href="stocks-timeline.html">Timeline →</a><a href="stocks-validation.html">Prospective Validation →</a></div><div class="stock10c-tabs" id="stock10cTabs"></div><div class="card"><div id="stock10cPane" class="stock10c-empty">Loading Stocks Decision Intelligence. No BUY/SELL or profit probability is generated.</div></div></div>`;
  ...
  function patchRoadmap(ready,frozen){
    document.querySelectorAll('.step').forEach(step=>{const key=step.querySelector('.stepN')&&step.querySelector('.stepN').textContent.trim();const status=step.querySelector('.status');if(!status)return;if(key==='10A'||key==='10B')status.textContent='COMPLETE';if(key==='10C')status.textContent=ready?'COMPLETE':'READY TO ACTIVATE';if(key==='10D')status.textContent=frozen?'FROZEN · ACTIVE':'READY TO ACTIVATE';});
  }
  ...
  function render(){
    ...
    const m=s.market_structure||{},f=s.fundamental_evidence||{},r=s.decision_readiness||{},rk=s.decision_risk||{},ai=s.ai_decision_reasoner||{},ct=s.counter_thesis||{};const drivers=driverItems(s);const ctf=ct.factors||[];const frozen=artifact.frozen===true&&artifact.freeze_validation&&artifact.freeze_validation.status==='MATCH';
    pane.className='';pane.innerHTML=`<div class="companyHead"><div class="companyTitle"><h2>${s.ticker} · ${s.name||''}</h2><p>${s.exchange||'—'} · Market session ${s.market_session||'—'} · SEC filing ${s.latest_sec_filing_date||'—'}</p></div><span class="stock10c-badge ${frozen?'ready':''}">${frozen?'MODEL FROZEN · '+String(artifact.model_hash||'').slice(0,10)+'…':'MODEL NOT FROZEN'}</span></div>
    ...
    <div class="stock10c-two"><div class="stock10c-panel"><h3>Why this view?</h3><div class="stock10c-list">${drivers.length?drivers.map(d=>`<div class="stock10c-item"><b>${d.label}</b> · ${d.text} · component ${Number(d.score).toFixed(0)}/100</div>`).join(''):'<div class="stock10c-item">No single component dominates the current view; evidence is balanced or incomplete.</div>'}</div></div><div class="stock10c-panel"><h3>Counter-Thesis · ${clean(ct.strength)}</h3><div class="stock10c-list">${ctf.length?ctf.map(x=>`<div class="stock10c-item ${String(x.severity||'').toLowerCase()}"><b>${clean(x.severity)}</b> · ${x.text}</div>`).join(''):'<div class="stock10c-item">No material opposing factor crossed the frozen thresholds.</div>'}</div></div></div>
    <div class="stock10c-guard"><b>10D guardrail:</b> Market View is an explainable evidence state, not BUY/SELL. Decision Readiness is not profit probability. ${frozen?'The Stage 10C methodology is frozen and hash-verified for prospective monitoring.':'Prospective validation remains blocked until the frozen methodology hash matches.'} Trade execution remains OFF.</div>`;renderTabs();
  }

  tabs.addEventListener('click',e=>{const b=e.target.closest('.stock10c-tab');if(!b)return;selected=b.dataset.t;render();});
  fetch('./data/stocks-decision-intelligence.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}).then(data=>{artifact=data;const symbols=Array.isArray(data.symbols)?data.symbols:[];const ready=symbols.length>0&&(data.status==='STOCKS_DECISION_INTELLIGENCE_READY'||data.status==='PARTIAL');const frozen=data.frozen===true&&data.freeze_validation&&data.freeze_validation.status==='MATCH';patchRoadmap(ready,frozen);if(!ready){badge.textContent=clean(data.status||'DECISION INTELLIGENCE NOT RUN');return;}badge.textContent=frozen?'FROZEN · PROSPECTIVE MONITORING':'PRE-VALIDATION INTELLIGENCE READY';if(frozen)badge.classList.add('ready');selected=symbols[0].ticker;renderTabs();render();}).catch(()=>{badge.textContent='DECISION DATA UNAVAILABLE';patchRoadmap(false,false)});
})();
